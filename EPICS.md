...
- **Deprecated `GrainWith*Key` marker interfaces** — retained and still meaning what they always did
  (`KeyKindFromMarker` maps each to its kind), so existing declarations compile unchanged. Removing
  the residual no-op `extends GrainWithStringKey` from test fixtures is cosmetic and unscheduled;
  `packages/parity` keeps them permanently, along with its `I` prefixes and Orleans FQN interface
  names, because those are upstream identifiers used for 1:1 re-diff.
