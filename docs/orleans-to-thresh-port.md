...
| `builder.UseLocalhostClustering(siloPort, gatewayPort, primarySiloEndpoint)` — several silos on one machine | `useStaticMembership(<the WHOLE view>)` + `useWebSocketTransport()`. Four things do not port. (1) The PRIMARY-ENDPOINT form has no counterpart: Orleans' development clustering GOSSIPS, so a joiner needs only the primary's address, while `useStaticMembership` takes the entire view — hand every silo the identical full list, and do not invent a "primary" key until a development/gossip membership provider exists. (2) There is NO GATEWAY PORT. A Thresh silo has ONE endpoint; a client (external or the embedded leg backing `createObjectReference`) dials it and is answered over that same duplex connection, so Orleans' second port has nothing to configure. (3) That one endpoint is BOTH the bind address and the advertised address, so a wildcard (`0.0.0.0`, `::`) is not configurable — it would be advertised to peers as the address to dial back. (4) PEER IDENTITY MUST BE DERIVED FROM THE ENDPOINT, by a rule every host in the cluster applies identically — `new SiloAddress(endpoint, "uid-" + endpoint, endpoint)` is the obvious one. `SiloAddress.ringKey` is `podName` alone, so silos sharing a constant name are ONE ring position however their ports differ; and because each silo must reconstruct its peers' addresses bit-identically to how those peers construct their own, a per-silo NAME knob cannot work (a peer cannot know a name only that peer's environment sets). Orleans is the same underneath — its ring identity is `IP:port:generation` and the silo name is cosmetic. |
...
| a hand-rolled gRPC **CLIENT** for driving a real server from a test (`GrpcChannel.ForAddress`, `Method<TReq,TResp>` descriptors, reflection-built `Marshaller`s) | the GENERATED client stub. `Grpc.Net.Client` needs a URI (`http://host:port`); `@grpc/grpc-js` needs `host:port` with NO scheme plus `ChannelCredentials.createInsecure()`, so a literal transliteration of the address string will not dial. Where the C# hand-rolls descriptors it is usually working around a C#-only problem — protos compiled `GrpcServices="Server"`, and a second client-side compile producing shadowing duplicate message types — which ts-proto's `outputServices=grpc-js` does not have: the `XxxServiceClient` already exists and shares its message types with the in-process service. Use it, and record the evaporated rationale as a deviation rather than reproducing the machinery. |
| a generated C# client's `await ...Async(req)`, `ResponseStream.MoveNext` loop, `RequestStream.WriteAsync`/`CompleteAsync` | grpc-js client calls are CALLBACK-based and must be promisified per call shape. Unary: `(err, resp) => err ? reject(err) : resolve(resp)`. Server-streaming: collect `data` and resolve on `end`, but REJECT on `error` — the C# loop THROWS out of `MoveNext` on an RPC failure, and a drain that resolves the rows received so far turns a server-side rejection into false agreement. Client-streaming: `write` each message in the C#'s order, then `end()`, settling from the completion callback. Build a FRESH `Metadata` per call (the C#'s `CallOptions` factory); one shared instance across concurrent calls is not the same object graph. |
| `CallOptions(headers: new Metadata { { "authorization", $"Bearer {key}" } })` | a `Metadata` from `@grpc/grpc-js` whose `authorization` entry is set to the same `Bearer <key>` string, passed as the call's metadata argument. |
...
| `await using var x = await Foo.CreateAsync(...)` in a test body | an explicit `const x = await Foo.create(...); try { ... } finally { await x.dispose(); }`. TypeScript has no `await using` at the target this port compiles to, and a disposable left to the end of the test leaks the resource on the first failing assertion — for a test cluster or a container that is an orphaned process, not a stale variable. |
...
| Testcontainers | `testcontainers` (the Node port), same API shape. Two things do not carry across: probe for the runtime with a BOUNDED `Promise.race` around `getContainerRuntimeClient()` at module load (a suite that HANGS on a missing Docker endpoint is worse than one that fails, and such a suite usually lands in the default test project), and keep the C#'s NARROW catch — skip only on the no-endpoint signal (`Could not find a working container runtime strategy`), and let image-pull and container-start failures ESCAPE, so a Docker-enabled machine can never silently skip a durability gate. Wait strategies rename rather than change: `Wait.ForUnixContainer().UntilMessageIsLogged(text)` -> `Wait.forLogMessage(text)`. Keep whichever strategy the C# chose — a distroless image (no shell) can never satisfy an INTERNAL probe, so swapping a log wait for a port/command wait does not just slow the start, it hangs it forever. |
| Orleans `ServiceId` used to ISOLATE storage rows between tests | it isolates only what the PROVIDER was built with. Since #59 `PostgresGrainStorage` keys rows by `(serviceId, grainId, stateName)` and `RedisGrainStorage` carries a service segment, but that id comes from the provider's own options — a silo's `serviceId` reaches them only through `SiloBuilder.addPostgresStorage`/`addRedisStorage`. A test that constructs a provider DIRECTLY over a shared pool passes no `serviceId`, so every such test lands on `DEFAULT_SERVICE_ID` and two tests over the same singleton grain key still COLLIDE. Either build the provider through the silo with a distinct `serviceId`, or give each test its own table, dropped in a `finally`. Note the default is `"default"` and NOT the cluster id: Orleans keeps `ServiceId` independent so it survives a redeployment that changes `ClusterId`. |
| a C# test-support class with NO `[Fact]`/`[Theory]` of its own (a fixture, a client, a shared comparison helper) | a plain `.ts` beside the suites, NEVER a `*.test.ts`. Vitest fails a `*.test.ts` that declares no suite, so a ledger row mapping a fixture to a test target produces a file that cannot exist as named. This has now been the correct answer five times in one port; it is a rule, not a case-by-case judgement. |

## Idioms that need judgement, not substitution

These are the places where a transliteration is wrong even when it compiles:

- **Raw string literals dedent; template literals do not.** A C# `"""` literal STRIPS the common
  leading whitespace of its lines, so an indented schema, SQL script or fixture document is emitted
  flush-left. A TypeScript template literal preserves every space. Write the text flush-left (or
  dedent it) rather than matching the C#'s source indentation: the bytes are what the parser, the
  hash, or the far side of a differential comparison sees.
