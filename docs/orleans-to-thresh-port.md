...
| `Array.Sort(SiloAddress[])` | `SiloAddress` has no comparator; sort a COPY by `toString()` ordinally. Only the identical ordering on every silo matters, not which order it is |
...
| `services.AddHostedService<T>()` (a service whose only job is to start something at silo boot) | `builder.addStartupTask(async (grains) => ...)`. The task is handed a `GrainFactoryAccess`, which also exposes `createObjectReference`, so anything that must mint an observer reference at boot belongs here. |
...
| grain **constructor** injection (`class FooGrain(IBar bar) : Grain`) | `builder.useGrainActivator({ createInstance })` plus an explicit options-bag interface per grain. Thresh types every grain constructor as zero-argument, so the bag is optional on the class and its absence is reported at first use. Fall through to `new ctor()` for grain types the activator does not know (the management grain among them). |
...
| `GrainCollectionOptions.ClassSpecificCollectionAge[typeof(TGrain).FullName] = age` | rewrite the grain class's `@grain()` metadata: `setGrainOptions(ctor, metadata.grainType, { ...metadata.options, collectionAgeSeconds })`. THRESH GAP: that registry is PROCESS-wide, so two silos in one process cannot hold different class-specific ages, where Orleans' map is per silo. Key by the class and its registered grain type, never by `class.name` (a bundler minifies it). |
