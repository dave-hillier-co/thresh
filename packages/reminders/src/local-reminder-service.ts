...
export class LocalReminderService {
  ...
  /** On a membership change: take the given ranges and (re)schedule owned reminders. */
  async refreshOwnership(ranges: readonly HashRange[]): Promise<void> {
    this.ranges = ranges;
    this.cancelAll();
    for (const [begin, end] of ranges) {
      for (const entry of await this.table.readRange(begin, end)) this.scheduleEntry(entry);
    }
  }
