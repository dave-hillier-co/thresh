...
// The TypeScript-first form: intersect the message surface with `GrainKey<TKey>`
// rather than extending a nominal `I`-prefixed marker interface. The two paths
// share one phantom field, so they must resolve identically.

describe("the GrainKey<TKey> marker", () => {
  it("maps an intersected marker to its getGrain key type", () => {
    type StringGrain = GrainKeyMarker<string> & { ping(): Promise<void> };
    type IntegerGrain = GrainKeyMarker<bigint> & { ping(): Promise<void> };
    type GuidGrain = GrainKeyMarker<Guid> & { ping(): Promise<void> };
    type GuidCompoundGrain = GrainKeyMarker<CompoundKey<Guid>> & { ping(): Promise<void> };
    type IntegerCompoundGrain = GrainKeyMarker<CompoundKey<bigint>> & { ping(): Promise<void> };

    expectTypeOf<GrainKeyFor<StringGrain>>().toEqualTypeOf<string>();
    expectTypeOf<GrainKeyFor<IntegerGrain>>().toEqualTypeOf<bigint>();
    expectTypeOf<GrainKeyFor<GuidGrain>>().toEqualTypeOf<Guid>();
    expectTypeOf<GrainKeyFor<GuidCompoundGrain>>().toEqualTypeOf<CompoundKey<Guid>>();
    expectTypeOf<GrainKeyFor<IntegerCompoundGrain>>().toEqualTypeOf<CompoundKey<bigint>>();
    expect(true).toBe(true);
  });

  it("recovers the same declared kind an Orleans marker would", () => {
    type IntegerGrain = GrainKeyMarker<bigint> & { ping(): Promise<void> };

    expectTypeOf<KeyKindFromMarker<IntegerGrain>>().toEqualTypeOf<"integer">();
    expectTypeOf<KeyKindFromMarker<IntegerGrain>>().toEqualTypeOf<
      KeyKindFromMarker<IIntegerKeyed>
    >();
    expect(true).toBe(true);
  });

  it("keeps the Orleans-compatible marker names as aliases of it", () => {
    // The `GrainWith*Key` interfaces now extend `GrainKey<TKey>`. Nothing may
    // observe the difference: ~246 existing usages depend on it.
    expectTypeOf<GrainWithStringKey>().toEqualTypeOf<GrainKeyMarker<string>>();
    expectTypeOf<GrainWithIntegerKey>().toEqualTypeOf<GrainKeyMarker<bigint>>();
    expectTypeOf<GrainWithGuidKey>().toEqualTypeOf<GrainKeyMarker<Guid>>();
    expectTypeOf<GrainWithGuidCompoundKey>().toEqualTypeOf<GrainKeyMarker<CompoundKey<Guid>>>();
    expectTypeOf<GrainWithIntegerCompoundKey>().toEqualTypeOf<
      GrainKeyMarker<CompoundKey<bigint>>
    >();
    expect(true).toBe(true);
  });

  it("defaults the type parameter to a string key", () => {
    type Greeter = KeyedGrain & { greet(name: string): Promise<string> };

    expectTypeOf<KeyedGrain>().toEqualTypeOf<GrainKeyMarker<string>>();
    expectTypeOf<GrainKeyFor<Greeter>>().toEqualTypeOf<string>();
    expect(true).toBe(true);
  });

  it("agrees with a kind declared on the definition instead", () => {
    type Marked = GrainKeyMarker<bigint> & { ping(): Promise<void> };
    const IIntersected = defineGrainInterface<Marked>("test.IIntersected");
    const IDeclared = defineGrainInterface<IUnmarked, "integer">("test.IDeclaredKind", {
      key: "integer",
    });

    expectTypeOf<GrainRuntime["getGrain"]>().toBeCallableWith(IIntersected, 1n);
    expectTypeOf<GrainRuntime["getGrain"]>().toBeCallableWith(IDeclared, 1n);
    expect(IDeclared.key).toBe("integer");
  });
});
