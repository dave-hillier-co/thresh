...
/**
 * OpenTelemetry instruments for durable-job execution (`@thresh/durable-jobs`):
 * `thresh.durablejobs.started`/`.completed`/`.failed` counters for a job
 * attempt's outcome, and `thresh.durablejobs.queue.depth` — an observable gauge
...
function instrumentsOf(): Instruments {
  if (instruments === undefined) {
    ...
    instruments = {
      started: meter.createCounter("thresh.durablejobs.started", {
        description: "Durable job attempts started",
        unit: "{job}",
      }),
      completed: meter.createCounter("thresh.durablejobs.completed", {
        description: "Durable job attempts that completed successfully",
        unit: "{job}",
      }),
      failed: meter.createCounter("thresh.durablejobs.failed", {
...
export function registerDurableJobQueueDepth(queueDepth: () => number): () => void {
  const meter = metrics.getMeter("@thresh/observability");
  const gauge = meter.createObservableGauge("thresh.durablejobs.queue.depth", {
