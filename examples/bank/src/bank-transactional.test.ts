...
import { TellerGrain, TxAccountGrain } from "@thresh/example-bank/account-transactional";
...
function buildSilo(): SiloHost {
  return createSilo({ clusterId: "bank-tx", local })
    ...
    .registerGrain(TxAccountGrain)
    .registerGrain(TellerGrain)
    .build();
}
...
describe("transactional bank account (Phase 7)", () => {
  it("moves money between accounts atomically in one transaction", async () => {
    ...
    await silo.start();
    try {
      const teller = silo.getGrain(TellerGrain, "main");
      await teller.open("alice", 10_000);
      await teller.transfer("alice", "bob", 3_000);

      expect(await silo.getGrain(TxAccountGrain, "alice").balance()).toBe(7_000);
      expect(await silo.getGrain(TxAccountGrain, "bob").balance()).toBe(3_000);
    } finally {
      await silo.stop();
    }
  });
  ...
  it("rolls back the whole transfer when the debit overdraws (no half-apply)", async () => {
    ...
    try {
      ...
      expect(await silo.getGrain(TxAccountGrain, "alice").balance()).toBe(1_000);
      expect(await silo.getGrain(TxAccountGrain, "bob").balance()).toBe(0);
    } finally {
      await silo.stop();
    }
  });
