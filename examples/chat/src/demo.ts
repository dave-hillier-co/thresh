...
import type { ChatMessage } from "@thresh/example-chat/interfaces";
...
export async function runChatDemo(): Promise<ChatDemoResult> {
  ...
  try {
    ...
    for (const name of members) await silo.getGrain(ChatUserGrain, name).join("general");

    await silo.getGrain(ChatRoomGrain, "general").say("alice", "hello everyone");
    await tick();

    const fanOut: Record<string, ChatMessage[]> = {};
    for (const name of members) fanOut[name] = await silo.getGrain(ChatUserGrain, name).history();
    ...
    await silo.getGrain(ChatRoomGrain, "general").say("alice", "you around?");
    await silo.getGrain(ChatRoomGrain, "general").say("carol", "guess not");
    await tick();

    // Bob comes back and resumes its own subscription from where it left off.
    await silo.getGrain(ChatUserGrain, "bob").join("general");
    await tick();
    const bobHistory = await silo.getGrain(ChatUserGrain, "bob").history();
