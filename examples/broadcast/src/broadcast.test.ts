...
describe("broadcast channels (pub/sub fan-out acceptance)", () => {
  it("fans an alert out to every grain type subscribed to the channel's region", async () => {
    ...
    await silo.start();
    try {
      await silo.getGrain(AlertPublisherGrain, "ops").raise("eu", "disk pressure");
      ...
      const monitor = await silo.getGrain(RegionMonitorGrain, "eu").alerts();
      const audit = await silo.getGrain(AuditLogGrain, "eu").entries();
  ...
  it("isolates channels by key — a region hears only its own alerts", async () => {
    ...
    await silo.start();
    try {
      await silo.getGrain(AlertPublisherGrain, "ops").raise("eu", "eu-only");
      expect((await silo.getGrain(RegionMonitorGrain, "eu").alerts()).map((a) => a.text)).toEqual([
        "eu-only",
      ]);
      // The us monitor was never published to, so it has seen nothing.
      expect(await silo.getGrain(RegionMonitorGrain, "us").alerts()).toEqual([]);
    } finally {
      await silo.stop();
    }
  });
